...
**Generated:** 2026-05-28 (session snapshot, F3 + F4 + F11 + F5 + F96 + F76 work completed, F12/F15/F42/F82 by-design out-of-scope)
**Branch / Commit:** `refactor` @ `fcc41e9` (working tree contains F3 + F4 + F11 + F5 + F96 + F76 implementation, uncommitted)
**Reference taxonomy:** Master Catalog v4 (110 faults across categories A–O)
**Scope:** Every static-validation / interactive-prompt / SQL-emit-time hook that
catches a data-dependent fault. Tracks completion against v4 P0 + P1 (47 items).
**Verification snapshot:** `cargo test --workspace --exclude vespertide-fuzz` =
**2592 passed / 0 failed**, `cargo clippy -D warnings` = clean, `schemas/` drift = 0.
...
|---|---|---|
| ✅ **Implemented & verified** | **23** (P0: 14 / P1: 9) | Keep regression tests green |
...
| ❌ **Not implemented (in-scope)** | **12** (P0: 0 / P1: 12) | Primary work queue (P1 only) |
...
| F29 | CHECK expression strengthening | A | Detect drop+add of CHECK where new predicate is *strictly stricter* than old. AST comparison on the `<op>` family. |
