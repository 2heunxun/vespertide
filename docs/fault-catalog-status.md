...
**E category (Operational safety): F11 shipped (PostgreSQL NOT VALID); F94 shipped (runtime migration lock/statement timeouts); F12+F82 by-design; other P1 items (F15, F65) remain.**
...
## 5. ✅ Implemented & Verified — Full Inventory (21 items)
...
| **F94 (P1)** | **Migration timeout not set (runtime lock/statement timeouts)** | **Config-driven, applied at runtime by the `vespertide_migration!` macro path. `vespertide-config::VespertideConfig` gains `lock_timeout_ms` / `statement_timeout_ms: Option<u64>` (both `#[serde(skip_serializing_if = "Option::is_none")]`, wire-compatible). The macro reads them at compile time and bakes `run_embedded_migrations_with_options(.., MigrationRuntimeOptions::from_millis(..))` when set (else the original `run_embedded_migrations` call, unchanged). `vespertide::runtime::run_embedded_migrations_with_options` injects backend-appropriate statements at session start: PostgreSQL `SET LOCAL lock_timeout` / `SET LOCAL statement_timeout` (after BEGIN), MySQL `SET SESSION innodb_lock_wait_timeout` (ms→s, rounded up) / `max_execution_time`, SQLite `PRAGMA busy_timeout` (before BEGIN; no statement timeout). Original `run_embedded_migrations` preserved as a delegating wrapper → non-breaking. CLI `vespertide sql --transaction` / `vespertide log --transaction` opt-in flags wrap emitted SQL text in `BEGIN;`/`COMMIT;` via the existing `wrap_in_transaction` builder option. Schema regenerated; README documents the config + 3-backend mapping.** | **config tests 3 + macro codegen tests 3 + runtime-sqlite real-DB tests 2 (`with_options_applies_sqlite_busy_timeout`, `default_run_does_not_set_busy_timeout`) + CLI `--transaction` tests 3** |
