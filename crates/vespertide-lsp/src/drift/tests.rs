...
use vespertide_core::{SimpleColumnType, TableConstraint};
...
fn action_to_drift_create_table() {
    ...
    let (kind, byte_range, message) = action_to_drift(&action, &[], source, Some(&tree)).unwrap();
...
fn action_to_drift_add_column() {
    ...
    let source = r#"{"name":"user","columns":[{"name":"email","type":"text","nullable":false}]}"#;
    let tree = parse_json(source);
...
fn action_to_drift_modify_column_type() {
    ...
    let source = r#"{"name":"user","columns":[{"name":"id","type":"big_int","nullable":false}]}"#;
    let tree = parse_json(source);
...
fn action_to_drift_modify_column_nullable_false_to_true() {
    ...
    let source = r#"{"name":"user","columns":[{"name":"email","type":"text","nullable":true}]}"#;
    let tree = parse_json(source);
