...
fn refresh_discovers_all_models_from_config() {
    ...
    .unwrap();
    fs::write(
        models_dir.join("article.json"),
        r#"{"name":"article","columns":[{"name":"id","type":"integer","nullable":false,"primary_key":true},{"name":"author_id","type":"integer","nullable":false,"foreign_key":{"ref_table":"user","ref_columns":["id"]}}]}"#,
    )
    .unwrap();
