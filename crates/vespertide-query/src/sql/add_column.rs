...
pub fn build_add_column(
    ...
    if sqlite_needs_recreation {
        let table_def = current_schema.iter().find(|t| t.name == table).ok_or_else(|| QueryError::Other(format!("Table '{table}' not found in current schema. SQLite requires current schema information to add columns.")))?;
