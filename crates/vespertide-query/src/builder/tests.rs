...
fn test_add_column_with_fk_and_index_pair(#[case] title: &str, #[case] order: &[TableConstraint]) {
    let plan = plan_add_column_with_constraints(order);
    let schema = base_schema_no_constraints();
    let result = build_plan_queries(&plan, &schema).unwrap();
...
fn test_add_column_with_fk_and_unique_pair(#[case] title: &str, #[case] order: &[TableConstraint]) {
    let plan = plan_add_column_with_constraints(order);
    let schema = base_schema_no_constraints();
    let result = build_plan_queries(&plan, &schema).unwrap();
